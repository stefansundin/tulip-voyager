...
qboolean SV_UpdateServerCommandsToClient( client_t *client, msg_t *msg, qboolean allowPartial ) {
	...
	for ( i = client->reliableAcknowledge + 1 ; i <= client->reliableSequence ; i++ ) {
		// The original q3 msg overflow checks for 4 byte internally; we want to write svc_servercommand (1 byte), the index (4 byte) and the string
...
void SV_SendClientSnapshot( client_t *client ) {
	...
	MSG_Copy( &msgBak, (byte*)&msgBak_buf, sizeof(msgBak_buf), &msg );
