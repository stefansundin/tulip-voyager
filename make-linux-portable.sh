...
if [ $VERBOSE -eq 1 ] ; then
	...
	echo
	echo "tulipvoyded.$ARCH:"
	objdump -T build/release-linux-$ARCH/tulipvoyded.$ARCH | grep GLIBC | sed -e "s/.*GLIBC_/GLIBC_/g" | sort -t _ -k 2 -V
	echo
	echo "tulipvoyhm.$ARCH:"
	objdump -T build/release-linux-$ARCH/tulipvoyhm.$ARCH | grep GLIBC | sed -e "s/.*GLIBC_/GLIBC_/g" | sort -t _ -k 2 -V
	echo
	echo "tulipvoyhm_renderer_opengl1_$ARCH.so:"
	objdump -T build/release-linux-$ARCH/tulipvoyhm_renderer_opengl1_$ARCH.so | grep GLIBC | sed -e "s/.*GLIBC_/GLIBC_/g" | sort -t _ -k 2 -V
	echo
	echo "tulipvoyhm_renderer_opengl2_$ARCH.so:"
	objdump -T build/release-linux-$ARCH/tulipvoyhm_renderer_opengl2_$ARCH.so | grep GLIBC | sed -e "s/.*GLIBC_/GLIBC_/g" | sort -t _ -k 2 -V
	echo
...
GLIBC_SERVER=$(objdump -T build/release-linux-$ARCH/tulipvoyded.$ARCH | grep GLIBC | sed -e "s/.*GLIBC_//g" | cut -d' ' -f1 | sort -uV | tail -1)
GLIBC_C1=$(objdump -T build/release-linux-$ARCH/tulipvoyhm.$ARCH | grep GLIBC | sed -e "s/.*GLIBC_//g" | cut -d' ' -f1 | sort -uV | tail -1)
GLIBC_C2=$(objdump -T build/release-linux-$ARCH/tulipvoyhm_renderer_opengl1_$ARCH.so | grep GLIBC | sed -e "s/.*GLIBC_//g" | cut -d' ' -f1 | sort -uV | tail -1)
GLIBC_C3=$(objdump -T build/release-linux-$ARCH/tulipvoyhm_renderer_opengl2_$ARCH.so | grep GLIBC | sed -e "s/.*GLIBC_//g" | cut -d' ' -f1 | sort -uV | tail -1)
